...
\end{frame}
	...
	\begin{frame}
		...
		\resizebox{\textwidth}{!}{
			\begin{tabular}{|p{4cm}|p{10cm}|}  % l-влево, c-по центру, r-
				...
				sum(x) & Сумма \\
				\hline
				diff(x, lag=n) & Разности между значениями в выборке, взятыми с заданным интервалом lag. По умолчанию интервал равен 1 \\
				\hline
				min(x) & Минимум \\
				\hline
				max(x) & Максимум \\
				\hline
				scale(x, center=TRUE, scale=TRUE) & Значения объекта x, центрированные (center=TRUE) или стандартизированные (center=TRUE, scale=TRUE) по столбцам \\
				\hline
			\end{tabular}
		}		
	...
	\begin{frame}[fragile]
		...
		{\fontsize{8}{9}\selectfont
		\begin{verbatim}
# Функция для фильтрации генов по экспрессии и длине
filter_genes <- function(gene_data, 
			              min_expression = 1.0, 
			              min_length = 1000, 
			              chromosome = NULL) {
				
    # Фильтрация по уровню экспрессии
    filtered <- gene_data[gene_data$expression >= min_expression, ]
				
    # Фильтрация по длине гена
    filtered <- filtered[filtered$length >= min_length, ]
				
    # Фильтрация по хромосоме (если указана)
    if (!is.null(chromosome)) {
        filtered <- filtered[filtered$chr == chromosome, ]
    }
...
genes <- data.frame(
    ...
    expression = runif(100, 0, 10),
    length = sample(500:5000, 100, replace = TRUE),
    chr = sample(c("chr1", "chr2", "chr3"), 100, replace = TRUE)
)
				
# Используем функцию с разными аргументами
...
default_filtered <- filter_genes(genes)
			\end{verbatim}
		}
	\end{frame}
...
\textbf{Область видимости} (scope) — это контекст, в котором переменные могут быть видимыми и доступны для использования. В языке R существует несколько уровней областей видимости: глобальная и локальная.
{\fontsize{10}{10}\selectfont
		\begin{verbatim}
    x <- "глобальная"
	...
	\begin{frame}[fragile]
		\frametitle{Работа с окружениями}
		\textbf{Окружение} (environment) - это область памяти, где хранятся объекты (переменные, функции), созданные или загруженные в текущую сессию. R находит переменные сначала в локальном окружении, затем в родительском, а затем и выше, до базового окружения.
		{\fontsize{8}{8}\selectfont
			\begin{verbatim}
    x <- 10
    myenv <- new.env()
    assign("x", "Hello", env=myenv)
	...
	\begin{frame}[fragile]
		...
		В языке R даты обычно вводятся в виде текстовых строк, а затем преобразуются в формат даты и хранятся в числовом виде. Для этого преобразования используется функция as.Date(), где x – это дата в текстовом формате, а input\_format определяет формат представления даты.
		{\fontsize{9}{10}\selectfont
		\begin{verbatim}
	...
	\begin{frame}[fragile]
		\frametitle{POSIXct и POSIXlt}
		\textbf{POSIXct} (Calendar Time) хранит время как число секунд с 1970-01-01 00:00:00.
		
		\textbf{POSIXlt} (Local Time) хранит время в виде списка с отдельными полями (год, месяц, день, час и т.д.)
		{\fontsize{8}{9}\selectfont
			\begin{verbatim}
				time_ct <- as.POSIXct("2024-12-16 15:30:00")
				time_lt <- as.POSIXlt("2024-12-16 15:30:00")
				
				unclass(time_ct)[1]  # 1734363000
				unclass(time_lt)[c("hour", "min", "sec")] # hour=15, min=30, sec=0 
				
				# Извлечение компонентов (одинаково для обоих типов)
				format(time_ct, "%H:%M")   # "15:30"
				format(time_lt, "%H:%M")   # "15:30"
				
				# Но для POSIXlt можно обратиться напрямую
				time_lt$hour  # 15
				time_lt$mon   # 11 (0-январь, 11-декабрь)
			\end{verbatim} 	
		}
	\end{frame}
