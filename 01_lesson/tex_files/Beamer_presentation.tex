...
\begin{frame}[fragile]
		...
		\begin{itemize}
			\item \textbf{numeric} – переменные, содержащие целочисленные значения (integer), действительные числа (double) и комплексные числа (complex);
	...
	\begin{frame}[fragile]
		\frametitle{Типы данных в R}
		\begin{itemize}
			\item \textbf{logical} – переменные, содержащие логические значения: FALSE (сокращенно F) и TRUE (T);
			{\fontsize{10}{10}\selectfont
			\begin{verbatim}
			...
			\item \textbf{character} – текстовые переменные (отдельные значения таких переменных задаются в двойных либо одинарных кавычках);
			{\fontsize{10}{10}\selectfont
			\begin{verbatim}
	...
	\begin{frame}[fragile]
		\frametitle{Векторы}
		\textbf{Векторы} – это одномерные объект, которые могут хранить числовые, текстовые или логические значения (комбинации не допускаются)
		{\fontsize{10}{10}\selectfont
		\begin{verbatim}
